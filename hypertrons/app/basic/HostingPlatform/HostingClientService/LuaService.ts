...
import { Repo, RepoDir, RepoFile } from '../../DataTypes';
...
> Error: ` + message;
  ...
  protected lua_createOrUpdateFile(filePath: string, content: string, commitMessgae: string, branchName: string, cb: () => void): void {
    this.app.logger.info('enter createOrUpdateFile-----------------');
    this.app.logger.info('branchName-----------------', branchName);
    this.client.createOrUpdateFile(filePath, content, commitMessgae, branchName, cb);
  }

  @luaMethod()
  protected lua_newPullRequest(title: string, body: string, head: string, base: string, allowModify: boolean): void {
    this.app.logger.info('enter newPullRequest-----------------');
    this.client.newPullRequest({ title, body, head, base, allowModify });
  }
  ...
  @luaMethod()
  /*protected lua_translateMetrics(src: string, to: string): Promise <TranslateResult> {*/
  protected lua_translateMetrics(src: string, to: string): Promise<string> {
    this.logger.info(`Gonna translate to ${to}, src = ${src}`);

    return new Promise(async resolve => {
      this.logger.info('Enter Promise function ...');
      const result = await this.app.translate.translate('AIzaSyA83zmNrz8rwU4VPNVKtMazOoA7arl1cOI', src, to);
  ...
  /*protected lua_translate(src: string, to: string, cb: (res: string) => void): string {*/
    this.logger.info('Gonna get config for translation ...');
    this.logger.info(`Gonna translate to ${to}, src = ${src}`);
    const strings = src.split(os.EOL);
    this.app.translate.translate('AIzaSyA83zmNrz8rwU4VPNVKtMazOoA7arl1cOI', strings, to).then(res => cb(res.map(r => r.translatedText).join(os.EOL)));
    // tslint:disable-next-line:no-unused-expression
  }

  @luaMethod()
  protected lua_getDirectoryContent(path: string): Promise<RepoDir[] | undefined> {
    this.logger.info('enter-----------------------------', path);
    return new Promise(async resolve => {
      const result = await this.client.getDirectoryContent(path);
      resolve(result);
    });
  }

  @luaMethod()
  protected lua_getFileContent(path: string): Promise<RepoFile | undefined> {
    this.logger.info('enter-----------------------------', path);
    return new Promise(async resolve => {
